...
typedef struct {
    iperf_cfg_t cfg;
    bool finish;        /* set by iperf_stop() / time limit to abort the whole test */
    bool conn_finish;   /* set when a single client connection ends (server keeps running) */
    uint32_t actual_len;
    uint32_t buffer_len;
    uint8_t *buffer;
    uint32_t sockfd;
...
static void iperf_report_task(void *arg)
{
    ...
    printf("\nInterval       Bandwidth\n");
    while (!s_iperf_ctrl.finish && !s_iperf_ctrl.conn_finish) {
        ...
        k++;
        /* time == 0 means "run forever until stopped by command" (server mode) */
        if (time != 0 && cur >= time) {
            printf("%2d.0-%2d.0 sec  %.2f %cbits/sec\n", 0, time, average, format_ch);
            /* configured duration reached: stop the traffic loop */
...
IRAM_ATTR static void socket_recv(int recv_socket, struct sockaddr_storage listen_addr, uint8_t type)
{
    ...
    /* reset per-connection counters so each new client gets a clean report */
    s_iperf_ctrl.actual_len = 0;
    s_iperf_ctrl.conn_finish = false;
    while (!s_iperf_ctrl.finish && !s_iperf_ctrl.conn_finish) {
        ...
        if (actual_recv < 0) {
            ...
            /* client gone or idle timeout: end this connection only, keep server alive */
            s_iperf_ctrl.conn_finish = true;
            break;
...
static esp_err_t iperf_run_tcp_server(void)
{
    ...
    /* Keep accepting clients until the whole test is aborted with 'iperf --abort' */
    while (!s_iperf_ctrl.finish) {
        if (s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV6) {
#if IPERF_IPV6_ENABLED
        ...
        if (client_socket < 0) {
            /* accept() idle timeout or transient error; keep waiting unless aborted */
            continue;
        }
        if (s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV6) {
#if IPERF_IPV6_ENABLED
        ...
        ESP_LOGI(TAG, "TCP client finished; waiting for next client (run 'iperf --abort' to stop)");
    }
...
static esp_err_t iperf_run_udp_server(void)
{
    ...
    /* Keep serving datagrams until the whole test is aborted with 'iperf --abort' */
    while (!s_iperf_ctrl.finish) {
