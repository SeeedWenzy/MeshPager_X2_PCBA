...
bleprph_on_sync(void)
{
    ...
    char adv_str[32] = {0};
    char *fix_str = "wio-esp32s3-";
    memcpy(adv_str, fix_str, strlen(fix_str));
    sprintf(adv_str + strlen(fix_str), "%02x%02x", addr_val[1], addr_val[0]);
    rc = ble_svc_gap_device_name_set(adv_str);
    assert(rc == 0);
