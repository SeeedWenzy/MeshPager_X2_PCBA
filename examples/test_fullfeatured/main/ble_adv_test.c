...
esp_err_t ble_adv_test_init(void)
{
    ...
    /* Set device name */
    ble_svc_gap_device_name_set("test_fullfeatured");
...
esp_err_t ble_adv_test_start(void)
{
    ...
    /* Idempotent: ble_init may be called more than once (boot init + the
     * runner's lazy init). If advertising is already active, treat as success
     * instead of returning BLE_HS_EALREADY. */
